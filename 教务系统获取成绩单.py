...
def getScores(username, password, filename=None):
    #站点访问需要的信息
    website = {
        "publickey": {
        ...
        "scores": {
            "url": f"https://jwxt.cjlu.edu.cn/cjcx/cjcx_cxDgXscj.html?doType=query&gnmkdm=N100801&su={username}",
            "headers": {
                "Host": "jwxt.cjlu.edu.cn",
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:91.0) Gecko/20100101 Firefox/91.0",
                "Accept": "application/json, text/javascript, */*; q=0.01",
                ...
                "Connection": "keep-alive",
                "Referer": f"https://jwxt.cjlu.edu.cn/xsxxxggl/xsgrxxwh_cxXsgrxx.html?gnmkdm=N100801&layout=default&su={username}",
                "Sec-Fetch-Dest": "empty",
                "Sec-Fetch-Mode": "cors",
                "Sec-Fetch-Site": "same-origin",
            },
    ...
    try:
        ...
        if 200 == response.status_code:
            ...
            if not filename: filename = f"{username[-4:]}_{str(time.time())[4:10]}.csv"
            with open(filename, 'w', encoding='utf-8', newline='') as f_obj:
                csv_writer = csv.writer(f_obj)
                csv_writer.writerow(["学年", "学期", "课程代码", "课程名称", "学分", "成绩", "考试性质", "绩点", "课程性质"])
                for score in scores:
                    jd = float(score.get("jd", "0"))
                    if jd:
                        jd_avg.append(jd)
                        xf_z += float(score.get("xf", "0"))
                    csv_writer.writerow([score.get("xnmmc", None), score.get("xqmmc", None), \
                                         score.get('kch', None), score.get("kcmc", None), \
                                         score.get("xf", None), score.get("cj", None), \
                                         score.get("ksxz", None), score.get("jd", None), score.get("kcxzmc", None),])
            print(f"成绩获取成功! (^ v ^)\n\t{username}\t->\t总学分: {xf_z}\t平均绩点: {sum(jd_avg)/len(jd_avg):.2f}\n")
        else:
            print(f"成绩获取失败! (V ~ V)\n\t{username}\n")
