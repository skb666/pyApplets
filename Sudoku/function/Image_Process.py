...
def correct(img_original):
    ...
    contour,hierarchy=cv2.findContours(close,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    for cnt in contour:
    ...
    contour,hierarchy=cv2.findContours(res,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
    centroids=[]
    for cnt in contour:
...
def find_biggest_bounding_box(im_number_thresh):
    ...
    """
    # 轮廓检测
    contour, hierarchy1 = cv2.findContours(im_number_thresh.copy(),
            cv2.RETR_EXTERNAL,
